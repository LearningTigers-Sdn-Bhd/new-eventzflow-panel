...
function printHeader() {
	...
	console.log();
	console.log(
		c.dim + "  Convert images to WebP format with optimized quality" + c.reset,
	);
...
async function optimizeDirectory(dirPath, displayName) {
	...
	console.log(
		c.dim + "  Target" + c.reset + "     " + c.white + displayName + c.reset,
	);
	console.log(
		c.dim +
			"  Images" +
	...
	console.log(
		c.dim +
			...
			"   " +
			c.white +
			`${MAX_WIDTH}px max · ${QUALITY}% quality` +
			c.reset,
	);
	console.log(
		c.gray +
...
async function main() {
	...
	console.log(
		c.dim +
			...
			"  " +
			c.green +
			`${grandTotalSuccess}/${grandTotalFiles} images` +
			c.reset +
			c.gray +
			"   " +
			c.reset +
