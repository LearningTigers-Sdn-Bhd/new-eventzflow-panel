...
export function useScanHistory() {
	...
	const { 
		...
		trpc.event.ticket.getMyScannedTickets.queryOptions({ 
			limit: 1000 // Get last 1000 scanned tickets
		})
	);
	...
	const scanResults = useMemo(() => {
		const allScans: ScanResult[] = [
			...
			...backendTickets.map((ticket) => ({
				ticketId: ticket.id,
				timestamp: ticket.checkInAt ? new Date(ticket.checkInAt) : new Date(ticket.createdAt),
				...
				seatNumber: ticket.customLabels?.find((l) => l.name.toLowerCase().includes("seat"))?.value,
		...
		if (result.status === "success") {
			...
			setTimeout(() => {
				refetch().then(() => {
					...
					setLocalScans((prev) => prev.filter((s) => s.ticketId !== result.ticketId));
				});
	...
	const getStats = useCallback(() => {
		...
		const duplicates = scanResults.filter((r) => r.status === "duplicate").length;
