 ...
 * Handles ticket check-in validation via tRPC (online) or localStorage (offline)
 */
...
import { ERROR_MESSAGES, SUCCESS_MESSAGES } from "@/components/pages/scan/constants";
...
export function useTicketValidation() {
	...
	const validateTicket = useCallback(
		async (ticketId: string, scannedTicketIds: Set<string>): Promise<ScanResult> => {
			...
			try {
				...
				const checkedInTicket = await trpcClient.event.ticket.checkInTicket.mutate({
					publicId: ticketId,
				});
				...
				if (isNetworkError && hasOfflineData()) {
					toast.warning("Network Error", { description: "Switching to offline mode" });
					const offlineResult = validateTicketOffline(ticketId, scannedTicketIds);
					if (offlineResult) {
						return offlineResult;
					}
