...
describe("kyClient 401 recovery", () => {
	...
	test("replays a POST request with its JSON body intact after refresh", async () => {
		...
		await expect(
			restClient.post<{ ok: boolean }>("v1/protected", { foo: "bar" }),
		).resolves.toEqual({ ok: true });
		expect(forceRefreshCalls).toBe(1);
		expect(bodies).toEqual([{ foo: "bar" }, { foo: "bar" }]);
		expect(authHeaders).toEqual(["Bearer stale-token", "Bearer fresh-token"]);
	});
