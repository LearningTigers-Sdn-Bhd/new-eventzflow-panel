...
import {
	...
	createPublicBooking,
	type DetailedSlotsResponse, // Added import
 ...
 */
export const usePublicBookingDetails = (
	bookingId: string,
	bmEventId: string,
	eventId: string,
	...
	return useQuery<Booking, Error>({
		...
		enabled:
			!!bookingId && !!bmEventId && !!eventId && (options?.enabled ?? true),
		staleTime: 0,
		gcTime: 0,
		retry: false,
	});
	...
	return useQuery<AvailabilityResponse, Error>({
		...
		queryFn: async () => {
			if (!bmEventId || !eventId) {
				throw new Error(
					"bmEventId or eventId is missing for availability query.",
				);
			}
	...
	return useQuery<DetailedSlotsResponse, Error>({
		...
		queryFn: async () => {
			if (!bmEventId || !date || !eventId) {
				throw new Error(
					"bmEventId, date or eventId is missing for detailed slots query.",
				);
			}
	...
	>({
		...
		onSuccess: () => {
			...
			queryClient.invalidateQueries({
				queryKey: ["business-matching-host-availability"],
			});
