...
import type {
	...
	BackendImportVisitorsResponse,
	ImportTicketsResponse,
	ImportVisitorsResponse,
} from "./response";
...
/**
 * Import visitors from an Excel or CSV file
 */
export async function importVisitors(
	...
	try {
		...
		// Call the import endpoint
		const response =
			await restClient.postFormData<BackendImportVisitorsResponse>(
				url,
				formData,
			);
...
/**
 * Import visitors in dry-run mode (no writes) and return detailed report
 */
export async function importVisitorsDryRun(
	file: File,
	options?: { full?: boolean; noLabel?: boolean },
): Promise<ImportVisitorsResponse> {
	return importVisitors(file, { ...options, dryRun: true });
}
