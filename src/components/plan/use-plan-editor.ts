...
export function usePlanEditor(initialPlan: Plan) {
	...
	const batchMutation = useMutation({
		mutationFn: (objects: Partial<PlanObject>[]) =>
			batchUpdatePlanObjects(plan.id.toString(), objects),
		onSuccess: () => {
	...
	const planSettingsMutation = useMutation({
		mutationFn: (updates: Partial<Plan>) =>
			updatePlan(plan.id.toString(), updates),
	});
	...
	const updateObjects = useCallback(
		(updates: { id: number; updates: Partial<PlanObject> }[]) => {
			setPlan((prev) => {
