...
export default function LogoUpload({
	...
	useEffect(() => {
		...
		// Only call onChange if the file actually changed
		if (firstFile instanceof File && firstFile !== prevFileRef.current) {
			prevFileRef.current = firstFile;
			onChangeRef.current?.(firstFile);
		} else if (files.length === 0 && prevFileRef.current !== null) {
			// Only reset if we had a file before
	...
	return (
		<div className={cn("w-full space-y-2", className)}>
			<div
				...
				className={cn(
					"relative flex aspect-square flex-col items-center justify-center overflow-hidden rounded-lg border border-dashed transition-colors",
					isDragging
