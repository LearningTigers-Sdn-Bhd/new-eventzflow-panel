...
export function EditTicketTypeForm({
	...
	// Determine initial validity mode based on existing values
	const getInitialValidityMode = (): ValidityMode => {
		if (!ticketType.validFromDate && !ticketType.validToDate) {
			return "all";
		}
		// If from and to are the same, it's a single day
		if (ticketType.validFromDate === ticketType.validToDate) {
			return "single";
		}
	...
	const [validityMode, setValidityMode] = useState<ValidityMode>(getInitialValidityMode());
	const [selectedDate, setSelectedDate] = useState<Date | null>(
		ticketType.validFromDate && ticketType.validFromDate === ticketType.validToDate
			? parseISO(ticketType.validFromDate)
			: null
	);
	const [validFromDate, setValidFromDate] = useState<Date | null>(
		ticketType.validFromDate ? parseISO(ticketType.validFromDate) : null
	);
	const [validToDate, setValidToDate] = useState<Date | null>(
		ticketType.validToDate ? parseISO(ticketType.validToDate) : null
	);
