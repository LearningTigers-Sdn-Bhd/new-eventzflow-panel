...
import { BusinessMatchingEvent } from '@/lib/api/business-matching';
...
import { Link as LinkIcon, UserPlus, Loader2 } from "lucide-react";
...
const AttachHostDialog: React.FC<AttachHostDialogProps> = ({ bmEvent }) => {
  ...
  const { mutate: createHost, isPending: isCreating } = useCreateAndAssignHost(bmEvent.event_id);
  ...
  const handleCreateHost = async () => {
    ...
    createHost({
      ...
      onSuccess: (data) => {
        toast.success(`Host "${data.full_name}" created and assigned successfully!`);
        closeDialog();
      },
      onError: (error) => {
  ...
  return (
    <Tabs defaultValue="invite" className="w-full">
      ...
      <TabsContent value="invite" className="py-4">
        <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
                Share this link with the host to let them join this business matching session.
            </p>
      ...
      <TabsContent value="create" className="py-4">
        <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
                Create an account for the host and assign them to this session directly. Their email will be auto-verified.
            </p>
            <div className="space-y-2">
                <Label htmlFor="host-name">Host Full Name</Label>
                <Input id="host-name" value={hostName} onChange={(e) => setHostName(e.target.value)} placeholder="John Doe" />
            </div>
            <div className="space-y-2">
                <Label htmlFor="host-email">Host Email</Label>
                <Input id="host-email" type="email" value={hostEmail} onChange={(e) => setHostEmail(e.target.value)} placeholder="host@example.com" />
            </div>
            <div className="space-y-2">
                <Label htmlFor="host-phone">Host Phone (Optional)</Label>
                <Input id="host-phone" type="tel" value={hostPhone} onChange={(e) => setHostPhone(e.target.value)} />
            </div>
            <div className="space-y-2">
                <Label htmlFor="host-password">Password</Label>
                <Input id="host-password" type="password" value={hostPassword} onChange={(e) => setHostPassword(e.target.value)} />
            </div>
