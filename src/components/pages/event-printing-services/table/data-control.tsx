		...
		<div className="mb-4 flex flex-col border-y border-dashed bg-accent px-0 py-0 md:px-2 md:py-4 lg:px-4 lg:py-4">
			{!isTablet ? (
				<div className="hidden items-center gap-2 lg:flex">
					<QuerySearchField table={table} columns={searchColumns} placeholder="Search services..." />
					<DropdownMenu>
				...
				<div className="flex flex-col gap-2 lg:hidden">
					...
					<div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
						<Button
							variant="outline"
							onClick={() => table.getColumn(primarySortColumn)?.toggleSorting(table.getColumn(primarySortColumn)?.getIsSorted() === "asc")}
							className="rounded-none"
						>
							<ArrowDown className={cn("mr-2 h-4 w-4 transition-transform", table.getColumn(primarySortColumn)?.getIsSorted() === "asc" && "-rotate-180")} />
							{primarySortLabel}
						</Button>
						<Button
							variant="outline"
							onClick={() => table.getColumn(secondarySortColumn)?.toggleSorting(table.getColumn(secondarySortColumn)?.getIsSorted() === "asc")}
							className="rounded-none"
						>
							<ArrowDown className={cn("mr-2 h-4 w-4 transition-transform", table.getColumn(secondarySortColumn)?.getIsSorted() === "asc" && "-rotate-180")} />
							{secondarySortLabel}
						</Button>
						<Select value={`${table.getState().pagination.pageSize}`} onValueChange={(value) => table.setPageSize(Number(value))}>
							...
							<SelectContent className="rounded-none bg-background">
								{[10, 20, 30, 40, 50].map((pageSize) => (
									<SelectItem key={pageSize} value={`${pageSize}`} className="rounded-none">
										{pageSize} rows
									</SelectItem>
								))}
							</SelectContent>
