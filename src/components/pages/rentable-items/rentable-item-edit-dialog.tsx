...
interface RentableItemEditContentProps {
	item: RentableItem;
}
...
export function RentableItemEditContent({
	...
	const [categoryId, setCategoryId] = useState(String(item.itemCategoryId));
	...
	const updateMutation = useMutation({
		mutationFn: updateRentableItem,
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["rentable-items"] });
			toast.success("Rentable item updated successfully");
			closeDialog();
		},
		onError: (error: Error) => {
			toast.error("Failed to update rentable item", {
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		const removeImage = !image && !currentImageUrl && !!item.imageUrl;
		...
		<form onSubmit={handleSubmit} className="space-y-4">
			<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
				...
				<div className="space-y-2">
					...
					>
						<SelectTrigger className="h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 shadow-xs">
			...
			<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
				<div className="space-y-2">
					...
					<Input
						...
						placeholder="e.g., unit, sq ft, per day"
						disabled={isPending}
					/>
			...
			<div className="flex items-center justify-between">
				<div className="space-y-0.5">
					...
					<p className="text-muted-foreground text-sm">
						Inactive items won't be available for rental
					</p>
				</div>
				<Switch
					id="status"
