...
export default function EventSponsorshipsKanban({
	sponsorships,
	tiers,
}: EventSponsorshipsKanbanProps) {
	...
	return (
		<ScrollArea className="w-full whitespace-nowrap pb-4">
			<div className="flex gap-4">
				...
				{tiers.map((tier) => (
					<div key={tier.id} className="flex w-72 shrink-0 flex-col gap-3">
						...
						<div className="flex min-h-[200px] flex-col gap-3 rounded-lg border border-border/50 border-dashed bg-muted/20 p-2">
							...
							{groupedSponsorships[tier.id.toString()]?.length === 0 && (
								<div className="flex flex-1 items-center justify-center text-muted-foreground/50 text-xs italic">
									No sponsorships
								</div>
							)}
						</div>
