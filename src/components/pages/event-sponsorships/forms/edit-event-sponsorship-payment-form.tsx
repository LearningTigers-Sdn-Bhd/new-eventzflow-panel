...
import { DollarSign, Calendar, Hash, Trash2 } from "lucide-react";
...
import { useUpdateEventSponsorshipPayment, useDeleteEventSponsorshipPayment } from "@/hooks/use-event-sponsorships";
import type { EventSponsorship, EventSponsorshipPayment } from "@/lib/api/sponsorship/response";
...
export default function EditEventSponsorshipPaymentForm({ sponsorship, payment, onClose }: EditEventSponsorshipPaymentFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    received_at: payment.received_at ? new Date(payment.received_at).toISOString().split('T')[0] : "",
  ...
  const handleDelete = () => {
    openDialog({
      ...
      props: {
        ...
        description: `Are you sure you want to permanently delete this payment of ${sponsorship.currency} ${parseFloat(payment.amount).toLocaleString()}?`,
          ...
          <FieldGroup>
            ...
            <div className="space-y-3">
                <Field orientation="vertical">
                  <FieldLabel htmlFor={amountId}>Amount ({sponsorship.currency}) *</FieldLabel>
                ...
                <div className="text-xs text-muted-foreground space-y-1 pt-2 border-t">
                    {payment.created_by && (
                        <div>Created by {payment.created_by.full_name} on {new Date(payment.created_at).toLocaleDateString()}</div>
                    )}
                    {payment.updated_by && payment.updated_at !== payment.created_at && (
                        <div>Last updated by {payment.updated_by.full_name} on {new Date(payment.updated_at).toLocaleString()}</div>
                    )}
                </div>
            </div>
