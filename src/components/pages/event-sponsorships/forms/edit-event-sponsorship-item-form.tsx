...
export default function EditEventSponsorshipItemForm({
	sponsorshipId,
	item,
	currency,
	onClose,
}: EditEventSponsorshipItemFormProps) {
					...
					<FieldGroup>
						<div className="space-y-6">
							...
							<div className="space-y-1 border-t pt-2 text-muted-foreground text-xs">
								{item.created_by && (
									<div>
										Created by {item.created_by.full_name} on{" "}
										{new Date(item.created_at).toLocaleDateString()}
									</div>
								)}
								{item.updated_by && item.updated_at !== item.created_at && (
									<div>
										Last updated by {item.updated_by.full_name} on{" "}
