...
export function EventSponsorshipTiersTable<TData>({ data, onAddTier }: DataTableProps<TData>) {
            ...
            <div className="space-y-2">
              {table.getRowModel().rows?.length ? (
                table
                  .getRowModel()
                  .rows.map((row) => (
                    <EventSponsorshipTierItem key={row.id} tier={row.original as EventSponsorshipTier} />
                  ))
              ) : (
                <EmptyState
                  title="No tiers found"
              ...
              {table.getRowModel().rows?.length ? (
                table.getRowModel().rows.map((row) => (
                  <div key={row.id} className="col-span-1">
                    <EventSponsorshipTierItem tier={row.original as EventSponsorshipTier} />
                  </div>
                ))
              ) : (
