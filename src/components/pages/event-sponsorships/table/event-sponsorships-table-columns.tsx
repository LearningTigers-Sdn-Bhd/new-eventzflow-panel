...
export const generateColumns = (): ColumnDef<EventSponsorship>[] => {
	return [
				...
				return (
					...
					>
						...
						<span className="font-medium text-primary transition-colors group-hover:text-primary/80">
							{row.original.sponsor?.name || "Unknown Sponsor"}
						</span>
					</a>
				);
			},
		},
		{
			...
			header: "Tier",
			cell: ({ row }) =>
				row.original.tier_name_snapshot || (
		...
		},
		{
			...
			cell: ({ row }) => {
				const total = Number.parseFloat(
					row.original.total_sponsor_amount || "0",
				);
				const received = Number.parseFloat(row.original.received_total || "0");
