...
export function EventSponsorshipsTable<TData>({ data, onAddSponsorship }: DataTableProps<TData>) {
            ...
            <div className="space-y-2">
              {table.getRowModel().rows?.length ? (
                table
                  .getRowModel()
                  .rows.map((row) => (
                    <EventSponsorshipItem key={row.id} sponsorship={row.original as EventSponsorship} />
                  ))
              ) : (
                <EmptyState
                  title="No sponsorships found"
              ...
              {table.getRowModel().rows?.length ? (
                table.getRowModel().rows.map((row) => (
                  <div key={row.id} className="col-span-1">
                    <EventSponsorshipItem sponsorship={row.original as EventSponsorship} />
                  </div>
                ))
              ) : (
