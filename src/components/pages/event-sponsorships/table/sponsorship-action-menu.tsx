...
interface SponsorshipActionMenuProps {
  sponsorship: EventSponsorship;
}

export function SponsorshipActionMenu({ sponsorship }: SponsorshipActionMenuProps) {
  const params = useParams();
  const eventId = params.event_id as string;
  const router = useRouter();
  const { openDialog, closeDialog } = useDialog();
  const deleteMutation = useDeleteEventSponsorship();
  const { data: tiers } = useEventSponsorshipTiers(eventId);

  const handleView = () => {
    window.open(`/event/${eventId}/sponsorships/${sponsorship.id}`, '_blank');
  };
...
Archiving it will hide it and its related payments from active views. Are you sure?`
    ...
    if (confirm(warningMessage)) {
      deleteMutation.mutate(
        { eventId, id: sponsorship.id.toString() },
        {
