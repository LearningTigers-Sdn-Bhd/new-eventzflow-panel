...
import { updateResourceCategory } from "@/lib/api/resource/category";
...
export function EditCategoriesForm({ category }: EditCategoriesFormProps) {
	const { closeDialog } = useDialog();
	const queryClient = useQueryClient();

	const updateMutation = useMutation({
		mutationFn: updateResourceCategory,
		onSuccess: () => {
				...
				>
					<div className="flex flex-col gap-4">
						<form.Field
							...
							validators={{
								onChange: ({ value }) => {
									const result = editCategorySchema.shape.name.safeParse(value);
						...
						<form.Field name="description">
							{(field) => (
								<InputLabel
									...
									value={field.state.value}
									onChange={(value) => field.handleChange(value)}
									placeholder="e.g. Documentation for developers"
									disabled={isPending}
									type="textarea"
								/>
							)}
