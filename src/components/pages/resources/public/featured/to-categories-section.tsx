...
const ToCategoriesSection: React.FC = () => {
	...
	return (
		<section id="categories" className="bg-white px-6 py-16 md:px-12 md:py-30">
						...
						{isLoading ? (
							<div className="space-y-4">
								{["sk1", "sk2", "sk3", "sk4", "sk5"].map((id) => (
									<div key={id} className="border-black/10 border-b px-6 py-8">
										<div className="flex items-start gap-6">
											<Skeleton className="h-4 w-6 bg-black/10" />
											<div className="flex w-full flex-col gap-2">
												<Skeleton className="h-8 w-1/3 bg-black/10" />
											</div>
										</div>
									</div>
								))}
							...
							displayItems.map((item, i) => (
								<motion.div
									...
									className={`relative cursor-pointer overflow-hidden border-black/10 transition-all duration-300 ${
										hoveredIndex === i
