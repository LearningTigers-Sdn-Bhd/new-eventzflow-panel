...
// Use the same schema as API request but adaptable for form
...
export default function ShowLayout({ resource, onSuccess }: ShowLayoutProps) {
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            ...
            try {
                // Manual validation check if needed, but validators should handle it.
                // Tanstack form handleSubmit runs validation.
				...
				<div className="flex flex-col justify-center">
					<div className="rounded-2xl border bg-white p-8 shadow-sm md:p-10">
						...
						>
							...
							<form.Field
                                name="phone"
                                // Optional, but usually validation runs if value present.
                                validators={{
                                    onChange: ({ value }) => {
							...
							<form.Field
                                ...
                                validators={{
                                    onChange: ({ value }) => {
                                        const result = leadFormSchema.shape.company.safeParse(value);
                                        if (!result.success) return result.error.issues[0].message;
                                        return undefined;
                                    }
                                }}
