...
import { useWheel } from "@/hooks/surprise/shared/use-wheel";
...
const SpinWheel: React.FC<DrawProps> = ({
	...
	const {
		...
		{ participants, prizes, mode, onDrawComplete, isDrawing },
		{
			baseColors,
	...
	if (isEmpty) {
		return (
			<div className="flex aspect-square w-full items-center justify-center rounded-full bg-gray-50 font-mono text-gray-400">
				{mode === "prizes" ? "Add prizes" : "Add participants"}
			</div>
		);
	}
	...
	return (
		<div className="relative mx-auto flex w-full max-w-[450px] flex-col items-center justify-center">
			...
			<div className="relative aspect-square w-full max-w-[600px]">
					...
					<g transform={`translate(${width / 2}, ${height / 2})`}>
						...
						{arcs.map((d: d3.PieArcDatum<string>, i: number) => {
							...
							return (
								<g
									key={
										isVirtualMode ? `segment-${i}` : participant?.publicId || i
									}
								>
									<path
										...
										<g transform={`translate(${arcGenerator.centroid(d)})`}>
											...
											>
												...
												<text
													...
													style={{
														fontSize:
															internalParticipants.length > 12
																? "10px"
																: "14px",
													}}
												>
				...
				{isVirtualMode && flashingName && (
					<div className="pointer-events-none absolute inset-0 flex items-center justify-center">
						<div className="max-w-[70%] rounded-xl bg-linear-to-br from-orange-600 to-red-600 px-4 py-3 shadow-2xl">
			...
			{isVirtualMode && (
				<div className="mt-2 flex items-center gap-2 rounded-full border-2 border-orange-300 bg-linear-to-r from-orange-50 to-yellow-50 px-4 py-2 shadow-sm">
					<span className="font-semibold text-orange-700 text-sm">
						Drawing from
					</span>
					<span className="rounded-full bg-linear-to-r from-orange-500 to-red-500 px-3 py-1 font-bold text-sm text-white shadow-sm">
			...
			{onDraw && (
				<div className="mt-4 flex flex-col items-center gap-3">
					<button
						...
						className="rounded-xl border-4 border-orange-700 bg-linear-to-b from-orange-400 to-orange-600 px-10 py-3 font-black text-white text-xl uppercase tracking-wider shadow-[0_6px_0_0_#c2410c] transition-all hover:from-orange-300 hover:to-orange-500 active:translate-y-[4px] active:shadow-[0_2px_0_0_#c2410c] disabled:cursor-not-allowed disabled:border-gray-700 disabled:from-gray-400 disabled:to-gray-600 disabled:text-gray-300 disabled:shadow-[0_6px_0_0_#4b5563]"
					>
