...
import { routeMenuMap } from "@/components/sidebars/features/resources/resource-menu-config";
...
function ResourceLayoutContent({
	...
	// Use context from ResourceSidebarProvider - no more duplicate data fetching!
	const { resource, slug, isLoading } = useResourceSidebarContext();
					...
					<MobileStickyHeader>
						<MobileStickyHeaderMain>
							<MobileStickyHeaderRow>
								{(resource || slug) && (
									<MobileStickyHeaderIcon
										icon={<AvatarIcon title={resource?.title || slug || ""} />}
									/>
								)}
								<MobileStickyHeaderTitle>
									{resource
